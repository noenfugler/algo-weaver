...
class Backtest_ETV(Backtest):
    def __init__(self, **kwargs):
        if len(kwargs)>0:
            super(Backtest_ETV, self).__init__(**kwargs)
        else:
            super(Backtest_ETV, self).__init__()

    def graph(self, save):
        my_graph = Graph_Trend_Vigor(strategy=self.strategy, save=save)
    ...
    def initialise(self, **kwargs):
        ...
        self.strategy = Strategy_ehlers_trend_vigor_indicator(silent=False, allow_long=True, allow_short=False, bot=self)

        self.live=False

        indicator1 = Indicator_Ehlers_Trend_Vigor()
...
def main():
    ...
    my_backtest1 = Backtest_ETV(instrument=config1.instrument, interval=config1.interval, exchange=config1.exchange)
    my_backtest2 = Backtest_ETV(instrument=config2.instrument, interval=config2.interval, exchange=config2.exchange)
    my_backtest3 = Backtest_ETV(instrument=config3.instrument, interval=config3.interval, exchange=config3.exchange)
