...
class Backtest_mama_fama_cross(Backtest):
    def __init__(self, **kwargs):
        if len(kwargs)>0:
            super(Backtest_mama_fama_cross, self).__init__(**kwargs)
        else:
            super(Backtest_mama_fama_cross, self).__init__()

    def graph(self, save):
        my_graph = Graph_CCI_mama_fama_cross(strategy=self.strategy, save=save)
