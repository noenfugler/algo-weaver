...
class Backtest_Detrender(Backtest):
    def __init__(self, **kwargs):
        if len(kwargs)>0:
            super(Backtest_Detrender, self).__init__(**kwargs)
        else:
            super(Backtest_Detrender, self).__init__()

    def graph(self, save):
        my_graph = Graph_Detrender(strategy=self.strategy, save=save)
    ...
    def initialise(self, **kwargs):
        ...
        self.strategy = Strategy_Ehlers_Detrender(silent=False,allow_long=True, allow_short=False, bot=self)

        self.live=False

        indicator1 = Indicator_CCI()
        self.ls_indicators.append(indicator1)
        indicator2 = Indicator_WMA4()
        self.ls_indicators.append(indicator2)
        indicator3 = Indicator_Ehlers_Period()
        self.ls_indicators.append(indicator3)
        indicator4 = Indicator_Ehlers_Detrender()
        ...
        indicator2.create(self.dataset.data)
        indicator3.create(self.dataset.data, source1='wma4', source_re='cci_real', source_im='cci_imag')
        indicator4.create(self.dataset.data)
...
def main():
    ...
    # my_backtest1 = Backtest_Detrender(instrument=config1.instrument, interval=config1.interval, exchange=config1.exchange)
    # my_backtest2 = Backtest_Detrender(instrument=config2.instrument, interval=config2.interval, exchange=config2.exchange)
    my_backtest3 = Backtest_Detrender(instrument=config3.instrument, interval=config3.interval, exchange=config3.exchange)
