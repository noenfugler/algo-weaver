...
#Determines trend mode by a half cycle > last half cycle + 2.

class strategy_ACP_filt_trend(Strategy):
    def setup(self, source='binance'):
        instrument=''
    ...
    def execute(self):
        ...
        for row_num in range(self.warmup_candles, len(self.mydata.df)):
            if self.mydata.df.loc[row_num-1, 'filt_trend_mode'] == 1:
                message='Uptrend.  Go Long.'
                self.mydata.strategy_go_long(row_num, message=message)
            elif self.mydata.df.loc[row_num-1, 'filt_trend_mode'] == -1:
                if self.trade_short:        #go short
                    message='Downtrend.  Go Short.'
                    self.mydata.strategy_go_short(row_num, message=message)
                else:
                    message = 'Downtrend.  Exit position'
                    self.mydata.strategy_sell(row_num, message=message)
            else:
                message = 'Trend end.  Exit position'
                self.mydata.strategy_sell(row_num, message=message)
            ...
            if self.mydata.btc < 0.0: #short
                if self.mydata.df.loc[row_num-1,'high']/self.purchase_price > (1+self.stop_percent):
                    ...
                    print(self.mydata.df.loc[row_num,'date_time'], "Stop Sell", self.mydata.df.loc[row_num,'high'])
    ...
    def show_graph(self):
        ...
        fig.suptitle(self.interval + ' ACP filter Indicator ' + str(self.mydata.df['date_time'].tail(1).values[0][:19]))
...
my_strategy = strategy_ACP_filt_trend()
my_strategy.setup()
my_strategy.execute()
