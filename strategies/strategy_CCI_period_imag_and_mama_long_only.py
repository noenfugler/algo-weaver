...
class Strategy_CCI_period_imag_and_mama_long_only(Strategy):

    def should_long_setup_trigger(self):
        last_row = self.get_last_row_num()
        self.should_long_trigger = self.bot.data.loc[last_row - 2, 'cci_imag'] < self.bot.data.loc[last_row - 1, 'cci_imag'] and \
