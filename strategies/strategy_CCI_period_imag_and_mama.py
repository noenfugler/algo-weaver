...
class Strategy_CCI_period_imag_and_mama(Strategy):

    def __init__(self, **kwargs):
        if len(kwargs)>0:
            super(Strategy_CCI_period_imag_and_mama, self).__init__(kwargs)
    ...
    def should_long_cycle(self):
        last_row = self.get_last_row_num()
        return self.mode==0 and \
               self.bot.data.loc[last_row - 1, 'cci_imag'] > self.bot.data.loc[last_row - 1, 'cci_imag'] > self.trend_factor and \
               self.bot.data.loc[last_row - 2, 'cci_real'] / self.bot.data.loc[last_row - 1, 'cci_real'] > self.trend_factor
