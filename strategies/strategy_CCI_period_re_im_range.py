...
class Strategy_CCI_period_re_im_range(Strategy):

    def __init__(self, **kwargs):
        if len(kwargs)>0:
            super(Strategy_CCI_period_re_im_range, self).__init__(kwargs)
    ...
    def _should_long_cycle(self):
        return self.bot.data.loc[self.last_row, 'cci_mode_aj']==0 and self.trough_zone and self._up('mama') or \
               (self.bot.data.loc[self.last_row, 'cci_mode_aj']==0 and \
                self.bot.data.loc[self.last_row-1, 'cci_mode_aj'] != 0 and \
                self.bot.data.loc[self.last_row, 'mama'] > self.bot.data.loc[self.last_row-1, 'mama'])
    ...
    def _should_modify_long_cycle(self):
        return self.bot.data.loc[self.last_row, 'cci_mode_aj']==0 and self.peak_zone and self._down('mama') or \
               (self.bot.data.loc[self.last_row, 'cci_mode_aj']==0 and \
                self.bot.data.loc[self.last_row-1, 'cci_mode_aj'] != 0 and \
                self.bot.data.loc[self.last_row, 'mama'] < self.bot.data.loc[self.last_row-1, 'mama'])

    def _should_modify_long_setup_trigger(self):
        pass
